...
class Broker:
    ...
    def onMessage(self, client, userdata, msg):
        ...
        if msg.topic.startswith("meshlora/data"):
            ...
            try:
                payload_mqtt = ast.literal_eval(
                    msg.payload[7:].decode()
                    if b"!repost" == msg.payload[:7]
                    else msg.payload.decode()
                )
            except Exception as e:
                log(f"msg detected, but not a dict: {e!s}")
                return
            ...
            match result["status"]:
                case "keyseed":
                    if b"!repost" == msg.payload[:7]:
                        log("haven't received a Key Ack for a repost, dropping message")
                        return

                    log("got a Key Seed, 10-sec sleep for Key Ack and retry")
                    time.sleep(10)
    ...
    def send_message(self, topic, dst, message):
        ...
        self.CLIENT.publish(f"meshlora/data/{topic}", str(payload))
